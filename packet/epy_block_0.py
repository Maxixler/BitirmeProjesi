"""
Custom Header Parser for GNU Radio
HPD'den 32 bit header alip 'payload symbols' key'i ile mesaj gonderir.
protocol_parser_b yerine kullanilir.
"""
import numpy as np
...
class custom_header_parser(gr.sync_block):
    ...
    def work(self, input_items, output_items):
        ...
        while len(self.buf) >= 32:
            ...
            # Parse 16-bit length (repeated twice for verification)
            len1 = 0
            for i in range(16):
            ...
            if len1 == len2 and 0 < len1 < 10000:
                msg = pmt.make_dict()
                msg = pmt.dict_add(
                    msg, pmt.intern("payload symbols"), pmt.from_long(len1)
                )
