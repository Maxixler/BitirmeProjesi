...
def convert_markdown_to_latex(text):
    # First, handle code blocks because they should be verbatim and not processed further
    # We'll replace code blocks with a placeholder and store them, then reinsert later.
    # But for simplicity, we'll process line by line and handle code blocks as we did before.
    lines = text.split('\n')
    output = []
    i = 0
    in_code_block = False
    ...
    while i < len(lines):
        ...
        # We use a lookaround to ensure we don't match the second * in **
        ...
        if re.match(r'^\\s*[-*]\\s+', stripped):
            ...
            # We have already applied inline conversions to the whole line, but we removed the bullet.
            # However, the inline conversions were applied to the whole line, including the bullet.
            # Since we removed the bullet, we need to reapply inline conversions to the item_text?
            # Actually, the bullet was not part of the inline conversions (it's not surrounded by backticks or asterisks for bold/italic in a way that would be matched).
            # For safety, we can reapply the inline conversions to item_text, but we already did on the whole line and the bullet is plain text, so it should be fine.
