...
def process_line(line):
    # Remove longtable from \usepackage line: if the line contains \usepackage and longtable, remove longtable from the list
    if '\\usepackage' in line and 'longtable' in line:
        # We'll remove the word 'longtable' and any surrounding commas and spaces, then clean up.
        # Replace 'longtable' with empty string, then remove any double commas and spaces at the start/end of the list.
    ...
    # Pattern: \begin{longtable} [optional [ ... ]] {column spec}
    ...
    # But note: there might be spaces. We'll do a two-step: first remove the [] if present, then replace begin{longtable} with begin{tabular.
    # However, we can do it in one regex: match \begin{longtable} (with spaces) then optional [ ... ] then spaces then { ... } and replace with \begin{tabular and the same { ... }.
    # We'll use:
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python fix_latex2.py input.tex output.tex")
        sys.exit(1)
