...
*   **Warm-up Kayıplarının Minimize Edilmesi:** Symbol Synchronizer ve Costas Loop'un kilitlenmesi sırasında oluşan 1 warm-up paketi kaybının (lock-time loss) toplam BER/BLER üzerindeki yapay payı **%5.00'ten %0.10 (binde 1)** seviyesine çekilmiştir.
...
*   **1000 Paket BER/BLER Waterfall Sweep Başarısı:** 1000 paketlik dinamik izlemeli BER testi başarıyla koşturulmuştur. $8.4\text{ dB}$ SNR değerine kadar sabit binde 1 (%0.10) lock-loss sınırında çalışan sistemin, $7.5\text{ dB}$ SNR'da LDPC eşik değerinin aşılmasıyla User 1'de %0.20, User 2'de %0.40 BER seviyesine çıktığı ve teorik sınırı akademik olarak başarıyla doğruladığı gözlemlenmiştir.
