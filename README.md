...
└── usrp_noma/                  # Ana Python paketi
    ...
    │   ├── data_analysis.py     # IQ veri analizi ve özellik çıkarımı
    │   └── synthetic_data.py    # Sentetik IQ veri üretimi (DL eğitimi için)
    │
...
### Veri Seti Üretimi
```bash
python3 main.py generate-dataset --samples 300 --length 4096 --snr-min 0 --snr-max 25
```

### Model Eğitimi
```bash
# CNN modeli
python3 main.py train-model --data-dir data --model cnn --epochs 30

# ResNet modeli
python3 main.py train-model --data-dir data --model resnet --epochs 50
```

### IQ Veri Analizi
```bash
python3 main.py analyze-data --input data/sample_lora_868mhz.npy --freq 868e6
```

Çıktılar: training_curves.png, accuracy_vs_snr.png, confusion_matrix.png, iq_analysis.png
...
### `usrp_noma/analysis/data_analysis.py`
IQ verisi üzerinde kapsamlı analiz yapar. 25 boyutlu özellik vektörü çıkarımı, istatistiksel analiz ve 6 panelli görselleştirme.

### `usrp_noma/analysis/synthetic_data.py`
Deep Learning eğitimi için sentetik IQ verisi üretir. LoRa, NOMA, CW, OFDM ve gürültü sinyalleri oluşturur.

### `usrp_noma/deep_learning/models.py`
1D CNN (SignalClassifierCNN) ve 1D ResNet (SignalResNet) sinyal sınıflandırma modelleri.

### `usrp_noma/deep_learning/trainer.py`
DLTrainer sınıfı ile model eğitimi, validasyonu, test değerlendirmesi ve SNR bazlı performans analizi.
